...
let feedback = function (i, positivo_negativo){
    ...
    let col_img_feedback = $("<div></div>");
    col_img_feedback.addClass("col-12 col-sm-3");
    col_img_feedback.html(perguntas[i].pergunta);
    row.append(col_img_feedback);
    
    let col_text = $("<div></div>");
    col_text.addClass("col-12 col-sm-6");
    col_text.html("<p>"+ feed_texto + "</p>");
    row.append(col_text);


    text_feedback.append(row);
    // let imagem_feedback = "<div style='max-width: 300px; margin: auto;'>" + perguntas[i].pergunta + "</div>";
    $("#pergunta" + i).append(text_feedback);
    // $("#pergunta" + i).append(imagem_feedback);
